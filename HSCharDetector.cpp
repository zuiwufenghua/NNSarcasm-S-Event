...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
  ...
  NRMat<double> charhEmb;
  charhEmb.resize(m_charAlphabet.size(), m_options.wordEmbSize);
  charhEmb.randu(1001);

  m_classifier.init(wordEmb, wordhEmb, m_options.wordcontext, charEmb, charhEmb, m_options.charcontext, m_labelAlphabet.size(), m_options.wordHiddenSize, m_options.charHiddenSize, m_options.hiddenSize);
