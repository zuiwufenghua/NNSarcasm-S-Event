/*
 * CNNHSCharClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_CNNHSCharClassifier_H_
#define SRC_CNNHSCharClassifier_H_
...
public:
  ...
  Tensor<xpu, 2, double> _wordhEmb;
  Tensor<xpu, 2, double> _grad_wordhEmb;
  Tensor<xpu, 2, double> _eg2_wordhEmb;
  Tensor<xpu, 2, double> _ft_wordhEmb;
  hash_set<int> _wordh_indexers;
  ...
  Tensor<xpu, 2, double> _charhEmb;
  Tensor<xpu, 2, double> _grad_charhEmb;
  Tensor<xpu, 2, double> _eg2_charhEmb;
  Tensor<xpu, 2, double> _ft_charhEmb;
  hash_set<int> _charh_indexers;
  ...
  UniHidderLayer<xpu> _cnncharh_project;
  ...
  UniHidderLayer<xpu> _gatedchar_history_pooling;
...
public:
  ...
  inline void init(const NRMat<double>& wordEmb, const NRMat<double>& wordhEmb, int wordcontext, const NRMat<double>& charEmb, const NRMat<double>& charhEmb,
    ...
    _wordhEmb = NewTensor<xpu>(Shape2(_wordSize, _wordDim), 0.0);
    _grad_wordhEmb = NewTensor<xpu>(Shape2(_wordSize, _wordDim), 0.0);
    _eg2_wordhEmb = NewTensor<xpu>(Shape2(_wordSize, _wordDim), 0.0);
    _ft_wordhEmb = NewTensor<xpu>(Shape2(_wordSize, _wordDim), 1.0);
    assign(_wordhEmb, wordhEmb);
    for (int idx = 0; idx < _wordSize; idx++) {
      norm2one(_wordhEmb, idx);
    }

    _charhEmb = NewTensor<xpu>(Shape2(_charSize, _charDim), 0.0);
    _grad_charhEmb = NewTensor<xpu>(Shape2(_charSize, _charDim), 0.0);
    _eg2_charhEmb = NewTensor<xpu>(Shape2(_charSize, _charDim), 0.0);
    _ft_charhEmb = NewTensor<xpu>(Shape2(_charSize, _charDim), 1.0);
    assign(_charhEmb, charhEmb);
    for (int idx = 0; idx < _charSize; idx++) {
      norm2one(_charhEmb, idx);
    }

    _cnnchar_project.initial(_charHiddenSize, _char_cnn_iSize, true, 20, 0);
    _cnncharh_project.initial(_charHiddenSize, _char_cnn_iSize, true, 30, 0);
    _cnn_project.initial(_wordHiddenSize, _word_cnn_iSize, true, 40, 0);
    ...
    _gatedchar_pooling.initial(_charHiddenSize, _charHiddenSize, true, 70, 3);
    _gatedchar_history_pooling.initial(_charHiddenSize, _charHiddenSize, true, 80, 3);
    _tanh_project.initial(_hiddenSize, _poolmanners * _wordHiddenSize + _poolmanners * _token_representation_size, true, 90, 0);
    _olayer_linear.initial(_labelSize, _hiddenSize, false, 100, 2);
  ...
  inline void release() {
    ...
    FreeSpace(&_wordhEmb);
    FreeSpace(&_grad_wordhEmb);
    FreeSpace(&_eg2_wordhEmb);
    FreeSpace(&_ft_wordhEmb);
    FreeSpace(&_charhEmb);
    FreeSpace(&_grad_charhEmb);
    FreeSpace(&_eg2_charhEmb);
    FreeSpace(&_ft_charhEmb);
    _cnnchar_project.release();
    _cnncharh_project.release();
    _cnn_project.release();
    _gated_sent_pooling.release();
    _gated_history_pooling.release();
    _gatedchar_pooling.release();
    _gatedchar_history_pooling.release();
  ...
  inline double process(const vector<Example>& examples, int iter) {
    _eval.reset();
    _word_indexers.clear();
    _wordh_indexers.clear();
    _char_indexers.clear();
    _charh_indexers.clear();
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        for (int idy = 0; idy < word_num; idy++) {
          ...
          for (int idz = 0; idz < char_num; idz++) {
            ...
            charprime[idx][idy][idz][0] = (idx == seq_size - 1) ? (_charEmb[offset] / _ft_charEmb[offset]) : (_charhEmb[offset] / _ft_charhEmb[offset]);
          }
          ...
          for (int idz = 0; idz < char_num; idz++) {
            if (idx == seq_size - 1)
              ...
              _cnncharh_project.ComputeForwardScore(charinput[idx][idy][idz], charhidden[idx][idy][idz]);
          }
          ...
          if ((_charremove > 0 && _charremove != 4) || (_charremove < 0 && _charremove == -4) || _charremove == 0) {
            for (int idz = 0; idz < char_num; idz++) {
              if (idx == seq_size - 1)
                ...
                _gatedchar_history_pooling.ComputeForwardScore(charhidden[idx][idy][idz], chargateweight[idx][idy][idz]);
              }
            }
        ...
        for (int idy = 0; idy < word_num; idy++) {
          offset = words[idy];
          wordprime[idx][idy][0] = (idx == seq_size - 1) ? (_wordEmb[offset] / _ft_wordEmb[offset]) : (_wordhEmb[offset] / _ft_wordhEmb[offset]);
        }
        ...
        if (_b_wordEmb_finetune) {
          for (int idy = 0; idy < word_num; idy++) {
            ...
            if (idx == seq_size - 1) {
              ...
              _grad_wordhEmb[offset] += wordprimeLoss[idx][idy][0];
              _wordh_indexers.insert(offset);
            }
          }
        }
        ...
        for (int idy = 0; idy < word_num; idy++) {
          ...
          if ((_charremove > 0 && _charremove != 4) || (_charremove < 0 && _charremove == -4) || _charremove == 0) {
            ...
            for (int idz = 0; idz < char_num; idz++) {
              if (idx == seq_size - 1)
                ...
                _gatedchar_history_pooling.ComputeBackwardLoss(charhidden[idx][idy][idz], chargateweight[idx][idy][idz], chargateweightLoss[idx][idy][idz],
                    charhiddenLossTmp[idx][idy][idz]);
              }
          ...
          for (int idz = 0; idz < char_num; idz++) {
            if (idx == seq_size - 1)
              ...
              _cnncharh_project.ComputeBackwardLoss(charinput[idx][idy][idz], charhidden[idx][idy][idz], charhiddenLoss[idx][idy][idz],
                  charinputLoss[idx][idy][idz]);
            }
          }
          ...
          if (_b_charEmb_finetune) {
            for (int idz = 0; idz < char_num; idz++) {
              ...
              if (idx == seq_size - 1) {
                ...
                _grad_charhEmb[offset] += charprimeLoss[idx][idy][idz][0];
                _charh_indexers.insert(offset);
              }
            }
          }
        }
  ...
  void updateParams(double nnRegular, double adaAlpha, double adaEps) {
    ...
    _cnncharh_project.updateAdaGrad(nnRegular, adaAlpha, adaEps);
    ...
    _gatedchar_history_pooling.updateAdaGrad(nnRegular, adaAlpha, adaEps);
    ...
    if (_b_wordEmb_finetune) {
      ...
      for (it = _wordh_indexers.begin(); it != _wordh_indexers.end(); ++it) {
        int index = *it;
        _grad_wordEmb_ij = _grad_wordhEmb[index] + nnRegular * _wordhEmb[index] / _ft_wordhEmb[index];
        _eg2_wordhEmb[index] += _grad_wordEmb_ij * _grad_wordEmb_ij;
        tmp_normaize_alpha = F<nl_sqrt>(_eg2_wordhEmb[index] + adaEps);
        tmp_alpha = adaAlpha / tmp_normaize_alpha;
        _ft_wordEmb_ij = _ft_wordhEmb[index] * tmp_alpha * nnRegular;
        _ft_wordhEmb[index] -= _ft_wordEmb_ij;
        _wordhEmb[index] -= tmp_alpha * _grad_wordhEmb[index] / _ft_wordhEmb[index];
        _grad_wordhEmb[index] = 0.0;
      }
    ...
    if (_b_charEmb_finetune) {
      ...
      for (it = _charh_indexers.begin(); it != _charh_indexers.end(); ++it) {
        int index = *it;
        _grad_charEmb_ij = _grad_charhEmb[index] + nnRegular * _charhEmb[index] / _ft_charhEmb[index];
        _eg2_charhEmb[index] += _grad_charEmb_ij * _grad_charEmb_ij;
        tmp_normaize_alpha = F<nl_sqrt>(_eg2_charhEmb[index] + adaEps);
        tmp_alpha = adaAlpha / tmp_normaize_alpha;
        _ft_charEmb_ij = _ft_charhEmb[index] * tmp_alpha * nnRegular;
        _ft_charhEmb[index] -= _ft_charEmb_ij;
        _charhEmb[index] -= tmp_alpha * _grad_charhEmb[index] / _ft_charhEmb[index];
        _grad_charhEmb[index] = 0.0;
      }
  ...
  void checkgrads(const vector<Example>& examples, int iter) {
    ...
    checkgrad(examples, _cnncharh_project._W, _cnncharh_project._gradW, "_cnncharh_project._W", iter);
    checkgrad(examples, _cnncharh_project._b, _cnncharh_project._gradb, "_cnncharh_project._b", iter);
    ...
    checkgrad(examples, _gatedchar_history_pooling._W, _gatedchar_history_pooling._gradW, "_gatedchar_history_pooling._W", iter);
    checkgrad(examples, _gatedchar_history_pooling._b, _gatedchar_history_pooling._gradb, "_gatedchar_history_pooling._b", iter);

    if (_word_indexers.size() > 0)
      checkgrad(examples, _wordEmb, _grad_wordEmb, "_wordEmb", iter, _word_indexers);
    if (_wordh_indexers.size() > 0)
      checkgrad(examples, _wordhEmb, _grad_wordhEmb, "_wordhEmb", iter, _wordh_indexers);

    if (_char_indexers.size() > 0)
      checkgrad(examples, _charEmb, _grad_charEmb, "_charEmb", iter, _char_indexers);
    if (_charh_indexers.size() > 0)
      checkgrad(examples, _charhEmb, _grad_charhEmb, "_charhEmb", iter, _charh_indexers);
...
#endif /* SRC_CNNHSCharClassifier_H_ */
